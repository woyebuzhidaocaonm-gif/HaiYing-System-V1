"""Executable ROS 2 integration check using deterministic synthetic sensor data."""
...
class SyntheticSensor(Node):
    def __init__(self):
        super().__init__("synthetic_depth_sensor")
    ...
    def _publish(self):
        ...
        image = self.bridge.cv2_to_imgmsg(depth, encoding="16UC1")
        ...
        pixel.header.frame_id = "camera_color_optical_frame"
        pixel.point.x = 420.0
        pixel.point.y = 290.0
        pixel.point.z = 0.95
...
def main():
    ...
    try:
        ...
        if not np.allclose(actual, expected, atol=1e-6):
            ...
            return 1
        if source.result.header.frame_id != "camera_depth_optical_frame":
            print(
                f"FAIL: unexpected frame {source.result.header.frame_id}", file=sys.stderr
            )
            return 1
        print(
            "PASS: /vision/target_point = "
